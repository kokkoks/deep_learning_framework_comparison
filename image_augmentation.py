...
def random_affine(im):
    ...
    tform = AffineTransform(
        scale=(rand_range(0.9, 1.1), rand_range(0.9, 1.1)),
        rotation=rand_range(-0.25, 0.25),
        shear=rand_range(-0.15, 0.15),
        translation=(
            rand_range(-im.shape[0] // 15, im.shape[0] // 15),
...
def random_perspective(im):
    ...
    region = 1 / 6
    A = np.array(
        [[0, 0], [0, im.shape[0]], [im.shape[1], im.shape[0]], [im.shape[1], 0]]
    )
    B = np.array(
        [
            [
                int(rand_range(0, im.shape[1] * region)),
                int(rand_range(0, im.shape[0] * region)),
            ],
            [
                int(rand_range(0, im.shape[1] * region)),
                int(rand_range(im.shape[0] * (1 - region), im.shape[0])),
            ],
            [
                int(rand_range(im.shape[1] * (1 - region), im.shape[1])),
                int(rand_range(im.shape[0] * (1 - region), im.shape[0])),
            ],
            [
                int(rand_range(im.shape[1] * (1 - region), im.shape[1])),
...
def random_crop(im):
    ...
    margin = 1 / 10
    start = [
        int(rand_range(0, im.shape[0] * margin)),
        int(rand_range(0, im.shape[1] * margin)),
    ]
    end = [
        int(rand_range(im.shape[0] * (1 - margin), im.shape[0])),
